...
module.exports.extend = function(int17) {
  ...
  Messenger.prototype.loadMessages = function(original, callback) {
    ...
    try {
      ...
      fs.readFile(this.getLocaleFile(), config.encoding, function (err, data) {
        if (err) {
          if (config.fallback && original) {
            if (config.locale.length > 1) {
              config.locale.pop();
              return that.loadMessages(false, callback);
            } else {
              return that.int17.languages(config.locale[0], function (_err, children) {
                if (_err) return _.callOrThrow(that, callback, _err);

                if (children.length > 1) {
                  config.locale = children[1].split(R_LOCALE);
        ...
        try {
          config.messages = that.parse(data);
  ...
  Messenger.prototype.loadMessagesSync = function(original) {
    ...
    try {
      data = fs.readFileSync(this.getLocaleFile(), config.encoding);
    } catch (err) {
      if (config.fallback && original) {
        if (config.locale.length > 1) {
          config.locale.pop();
          ...
          var children = this.int17.languagesSync(config.locale[0]);
          if (children.length) {
            config.locale = children[0].split(R_LOCALE);
            return this.loadMessagesSync(false);
          }
        }
      }
    ...
    return config.messages = this.parse(data);
  };
  ...
  // Update the default options for this `Internationalization`.
  Internationalization.prototype.defaults = function(defaults) {
    return _.extend(defaults, { separator: Path.sep });
  };
      ...
      fs.readdir(dir, function (err, files) {
        ...
        files.forEach(function (file) {
          fs.stat(Path.join(dir, file), function (err, stats) {
            ...
            if (that.config.folders) {
              if (stats.isDirectory()) language = file;
            } else if (Path.extname(file) === that.config.extension) {
              language = Path.basename(file, that.config.extension);
            }
