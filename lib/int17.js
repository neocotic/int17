...
(function() {
  ...
  function getArgs(element) {
    return (element.getAttribute(ARGS_ATTRIBUTE) || '').split(';');
  }
  ...
  // Flatten all arguments passed into single array.
  function vargs() {
  ...
  // --------
      ...
      // processed.  
      // To specify arguments to be used as substitutions by a handler, that
      // attribute should have semi-colon separated values declared within an
      // `int17-args` attribute on the same element.
  var handlers = {
  ...
  // latter is used, the its `name` property specifies the name of the
  // localized message and the `args` property should be an array of arguments
  // to be used as substitutions when fetching the message.  
  // Any additional arguments are used as substitutions for numeric
  // placeholders, except where `args` properties specified.
  ...
  // localized message and the `args` property should be an array of arguments
  // to be used as substitutions when fetching the message.  
  // The resulting map will use name parts as keys (strings or `name` property
  // of objects).  
  // Any additional arguments are used as substitutions for numeric
  // placeholders, except where `args` properties specified.
  ...
  // The contents of `names` can be a mixture of strings and objects. When the
  // latter is used, the its `name` property specifies the localized message
  // for the option's HTML contents, the `value` property specifies the
  // actual value for the option, and the `args` property should be an array of
  // arguments to be used as substitutions when fetching the message.  
  // Any additional arguments are used as substitutions for numeric
  // placeholders, except where `args` properties specified.
