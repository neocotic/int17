...
(function() {
  ...
  // -----------------
      ...
      // Cache containing named instances of `Internationalization` which can
      // be shared.
  var cache = {}
  ...
  var handlers = {
          ...
          // for `names`.  
          // `names` should be comma separated values where each value is
          // treated as a message name.
  ...
  // language in the results will be the current locale, provided it isn't
  // filtered out by the `parent` argument. The only workaround for this is by
  // using the `languages` option during initialization.
  ...
  // To optimize the performance of this function, the languages are only
  // populated once.  
  // As the browser isn't aware of all available languages, the only language
  // in the results will be the current locale, provided it isn't filtered out
  // by the `parent` argument. The only workaround for this is by using the
  // `languages` option during initialization.
  ...
  // Build the public API which will be exposed later.
  var int17 = {
      ...
      // Remove all cached references to shared `Internationalization`
      // instances.
    , clearCache: function() {
        cache = {};
        return int17;
      ...
      // specified the cache will be checked for existing references and will
      // only create a new instance if one could not be found.
    , create: function(name) {
        if (name)
          return cache[name] = cache[name] || new Internationalization();
