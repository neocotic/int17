...
(function () {
  ...
  var DEFAULT_LOCALE = 'en'
    ...
    , R_WHITESPACE   = /\s/g
      // Name of the substitutions attribute used internally by the `handlers`.
    , SUBS_ATTRIBUTE = 'int17-subs';
  ...
  function getSubstitutions(element) {
    var subs = element.getAttribute(SUBS_ATTRIBUTE);
    return subs ? subs.split(';') : [];
  }
  ...
  Internationalization.prototype.traverse = function(element) {
    ...
    for (i = 0; i < children.length; i++) {
      ...
      if (this.messenger.clean) {
        child.removeAttribute(SUBS_ATTRIBUTE);
      }
    }
    return this;
  };
