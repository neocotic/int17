// [int17](http://neocotic.com/int17) 0.4.0  
...
(function () {
  ...
  var ENTITY_MAP      = {
    ...
    , R_ESCAPE        = (function () {
        ...
        for (var key in ENTITY_MAP) keys.push(key);
  ...
  (function () {
    var arr = SUBS_ATTRIBUTES, len = arr.length;

    for (var i = 0; i < len; i++) arr.push(arr[i].replace('i18n', 'int17'));
  })();
  ...
  _.extend = function(obj) {
    var i, prop, source, sources = _.slice(arguments, 1);
  ...
  _.pick = function(obj) {
    var key, keys = _.vargs(arguments, 1), result = {};
  ...
  var handlers = {
        ...
        , 'i18n-options': function(element, names) {
            ...
            var option, parts, subs = _.getSubstitutions(element);
        ...
        , 'i18n-values': function(element, values) {
            ...
            var expression, name, obj, paths, property, subs = _.getSubstitutions(element);
            ...
            for (var i = 0; i < values.length; i++) {
              ...
              if (name[0] === '.') {
                ...
                while (obj && paths.length > 1) obj = obj[paths.shift()];
                ...
                if (~SUBS_ATTRIBUTES.indexOf(name)) subs = _.getSubstitutions(element);
              }
            }
          }
  ...
  Messenger.prototype.deriveLocale = function() {
    var config = this.int17.config
      , locale = config.locale || this.extractLocale() || config.defaultLocale;
    config.locale = typeof locale === 'string' ? locale.split(R_LOCALE) : locale;
  ...
  Messenger.prototype.extractLocale = function() {
    ...
    return html && html.lang || navigator.language;
  };

  // Retrieve the named message from the bundle.
  Messenger.prototype.getMessage = function(messages, name) {
  ...
  Messenger.prototype.getLocaleFile = function(relative) {
    var config = this.int17.config
      , path   = this.getLocaleParent(true) + config.separator;
    path += config.folders ? config.fileName : config.locale.join('_');
  ...
  Messenger.prototype.getLocaleParent = function(relative) {
    var config = this.int17.config
      , path   = this.getLocaleRoot(true);
    if (config.folders) path += config.separator + config.locale.join('_');
  ...
  Messenger.prototype.getLocaleRoot = function(relative) {
    var path = this.int17.config.path;
  ...
  Messenger.prototype.loadMessages = function(original, callback) {
    ...
    try {
      if (!_.isEmpty(config.messages)) return _.callOrThrow(this, callback, null, config.messages);
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState === 4) {
          if (xhr.status === 200) {
            try {
              config.messages = that.parse(xhr.responseText);
              _.callOrThrow(that, callback, null, config.messages);
            } catch (err) {
              _.callOrThrow(that, callback, err);
            }
          } else {
            if (config.fallback && original && config.locale.length > 1) {
              config.locale.pop();
  ...
  Messenger.prototype.loadMessagesSync = function(original) {
    ...
    if (!_.isEmpty(config.messages)) return config.messages;
    ...
    if (xhr.status === 200) {
      return config.messages = this.parse(xhr.responseText);
    } else if (config.fallback && original && config.locale.length > 1) {
      config.locale.pop();
  ...
  function Internationalization() {
    ...
    function bindEscape(method) {
      that.escape[method] = function () {
        var result, alreadyEscaping = that.escaping;
        that.escaping = true;
    ...
    for (var i = 0; i < ESCAPE_METHODS.length; i++) bindEscape(ESCAPE_METHODS[i]);
  }
  ...
  Internationalization.prototype.attribute = function(selector, attr, name) {
    ...
    for (var i = 0; i < elements.length; i++) elements[i].setAttribute(attr, message);

    return this;
  };

  // Configure this `Internationalization` based on the specified `options`.  
  // Default configuration values will be used for any options that are missing or invalid.
  Internationalization.prototype.configure = function(options) {
    options = options || {};
    ...
    _.extend(this.config, defaults, _.pick(options, _.keys(defaults)));
    this.messenger.deriveLocale();
    this.config.languages.sort();
  ...
  Internationalization.prototype.content = function(selector, name) {
    ...
    for (var i = 0; i < elements.length; i++) elements[i].innerHTML = message;

    return this;
  };

  // Placeholder to allow override of default options without too much duplication.
  Internationalization.prototype.defaults = function(defaults) {
    return defaults;
  };
  ...
  Internationalization.prototype.get = function(name) {
    ...
    var subs    = _.vargs(arguments, 1)
      , message = this.messenger.getMessage(this.config.messages, name);

    message = message && this.substitute(message, subs);
  ...
  Internationalization.prototype.init = function(options, callback) {
    ...
    try {
      this.configure(options).messenger.loadMessages(true, function (err) {
        _.callOrThrow(that, callback, err);
      });
    } catch (err) {
      _.callOrThrow(this, callback, err);
  ...
  Internationalization.prototype.initSync = function(options) {
    this.configure(options).messenger.loadMessagesSync(true);
  ...
  Internationalization.prototype.languages = function(parent, callback) {
    ...
    try {
      _.callOrThrow(this, callback, null, this.languagesSync(parent));
    } catch (err) {
      _.callOrThrow(this, callback, err);
    }
  };
  ...
  Internationalization.prototype.languagesSync = function(parent) {
    ...
    if (!languages.length) languages.push(this.config.locale.join('-'));
  ...
  Internationalization.prototype.locale = function() {
    return this.config.locale.join('-');
  };
  ...
  Internationalization.prototype.options = function(selector, names) {
    ...
    for (var i = 0; i < elements.length; i++) {
      for (j = 0; j < names.length; j++) {
        ...
        if (_.isObject(name)) {
          if (name.value) option.value = name.value;
  ...
  Internationalization.prototype.property = function(selector, prop, name) {
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      if (obj) {
        obj[paths[0]] = message;
        if (paths[0] === 'innerHTML') this.traverse(element);
      }
    }
  ...
  // their values.
  Internationalization.prototype.substitute = function(message, substitutions) {
    var that         = this
    ...
    if (placeholders) {
      str = str.replace(R_PLACEHOLDER, function (placeholder, name) {
        var replacement = _.findPlaceholder(placeholders, name, that.config.ignoreCase);
  ...
  Internationalization.prototype.traverse = function(element) {
    if (typeof element === 'string') element = this.node.querySelector(element);
    element = element || this.node;
    if (!element) return this;

    var attribute, child, j, value
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      for (j = 0; j < attributes.length; j++) {
        ...
        if (value) {
          ...
          if (this.config.clean) child.removeAttribute(attribute);
        }
      }

      if (this.config.clean) {
        for (j = 0; j < SUBS_ATTRIBUTES.length; j++) child.removeAttribute(SUBS_ATTRIBUTES[j]);
      }
    }
  ...
  Int17.prototype.optimize = function(messages) {
    ...
    for (var message in messages) {
      ...
      for (placeholder in placeholders) delete placeholders[placeholder].example;
    }
